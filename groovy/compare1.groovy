	...
	for (int i = 0 ; i < 10; i++){
		for (int j = i + 1; j < 10; j++){
			...
			if ( ars[i].comparedTo == null ){
				...
				if (ars[j].comparedTo == null){
					ars[j].comparedTo = [];	
				}
			
				//println "compared before " + i + " " + j;
				def comparedToI = ars[i].comparedTo;
				def comparedToJ = ars[j].comparedTo;
				
				println ars[i].id + " " + comparedToI;
				println ars[j].id + " " + comparedToJ;
				
				//check to see if this target event has been compared to this alpha report
				def foundI = [ item_id: -1, score: -1 ];
				def foundJ = [ item_id: -1, score: -1 ];
				
				for (int k = 0; k < comparedToI.size(); k++){
					if (comparedToI[k].item_id == ars[j].id){
						foundI.score = comparedToI[k].score;
						foundI.item_id = comparedToI[k].item_id;
						break;
					}
				}
				
				for (int k = 0; k < comparedToJ.size(); k++){
					if (comparedToJ[k].item_id == ars[i].id){
						foundJ.score = comparedToJ[k].score;
						foundJ.item_id = comparedToJ[k].item_id;
						break;
					}
				}
